...
export const TaskProjectTimelineSheet: React.FC<TaskProjectTimelineSheetProps> = ({
  ...
  // Unsaved Changes Tracker & Discard Modal
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState(false);
  const [showDiscardModal, setShowDiscardModal] = useState(false);
  const [initialDrawerSnapshot, setInitialDrawerSnapshot] = useState('');
  ...
  useEffect(() => {
    ...
    const currentSnapshot = JSON.stringify({
      ...
      subVerificationNotes,
      subAdminComment,
      taskLink: (task as any)?.link_url || '',
      cardLink: (editingCard as any)?.link_url || '',
      listLink: (editingList as any)?.link_url || '',
    });
    if (!initialDrawerSnapshot) {
      setInitialDrawerSnapshot(currentSnapshot);
    } else if (currentSnapshot !== initialDrawerSnapshot) {
  ...
  const handleRequestCloseDrawer = (onConfirmClose?: () => void) => {
  ...
  const openDrawerForCard = (card: TaskCard, list?: TaskList) => {
    ...
    setSubLinkUrl((card as any).link_url && !(card as any).link_url.includes('example.com') ? (card as any).link_url : '');
    setSubAttachmentUrl((card as any).attachment_url || '');
  ...
  const openDrawerForList = (list: TaskList) => {
    ...
    setSubLinkUrl((list as any).link_url && !(list as any).link_url.includes('example.com') ? (list as any).link_url : '');
    setSubAttachmentUrl((list as any).attachment_url || '');
    setShowInvitePopover(false);
    setIsDrawerOpen(true);
  };
    ...
    try {
      if (editingCard) {
        await updateTaskCard(editingCard.id, {
          ...
          link_url: subLinkUrl.trim() || undefined,
          attachment_url: subAttachmentUrl.trim() || undefined,
        });
  ...
  const handleDeleteCard = async (cardId: string) => {
    try {
      ...
      alert(e.message || 'ลบการ์ดงานไม่สำเร็จ');
    }
  };
  ...
  const handleDeleteList = async (listId: string) => {
    try {
      ...
      alert(e.message || 'ลบบอร์ดไม่สำเร็จ');
    }
  };
  ...
  const [createListAssignees, setCreateListAssignees] = useState<string[]>([]);
  const [createListNote, setCreateListNote] = useState('');
  const [showCreateInvitePopover, setShowCreateInvitePopover] = useState(false);

  const handleAddListSubmit = async () => {
    if (!createListName.trim()) {
      alert('กรุณากรอกชื่อบอร์ดงาน');
      return;
    }
    ...
    try {
      const newList = await createTaskList(task.id, {
        ...
        assignee_ids: createListAssignees,
        description: createListNote.trim() || undefined,
      });

      if (createCardName.trim()) {
        await createTaskCard(newList.id, {
          title: createCardName.trim(),
      ...
      setCreateListAssignees([]);
      setCreateListNote('');
    } catch (e: any) {
      alert(e.message || 'สร้างบอร์ดงานไม่สำเร็จ');
    } finally {
  ...
  const loadSubItems = async (silent?: boolean) => {
            ...
            >
              <div className="flex flex-col items-center gap-1.5 pt-1">
                  <input
                    ...
                    className="w-full bg-transparent text-center text-xs font-bold text-blue-900 border-none p-1 focus:bg-white focus:ring-1 focus:ring-blue-500 rounded focus:outline-none"
                    title={list.name}
                  />
                </div>
            </td>
            <td colSpan={7} className="px-4 py-3 text-slate-400 italic text-xs">
              ยังไม่มีการ์ดงานย่อยในรายการนี้ (คลิกที่ชื่อโปรเจกต์เพื่อเพิ่ม)
            </td>
          </tr>
        ];
      }
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="bg-white border-2 border-slate-300 rounded-2xl shadow-xs overflow-hidden">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center ml-auto pl-3 border-l border-slate-200">
            <button
              type="button"
              onClick={handleAddList}
              disabled={isCreatingList}
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-xs transition-all cursor-pointer active:scale-95 disabled:opacity-50"
            >
              <Plus className="w-4 h-4" />
              <span>{isCreatingList ? 'กำลังสร้าง...' : 'เพิ่มบอร์ดงาน'}</span>
            </button>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-lg h-full shadow-2xl flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-200">
            ...
            <div className="bg-slate-900 text-white p-5 flex items-center justify-between border-b-4 border-blue-600">
              ...
              <div className="flex items-center gap-1">
                {(editingSubItem || editingCard || editingList) && (
                  <button
                    type="button"
                    onClick={() => {
                      if (editingSubItem) {
                        ...
                        if (confirm('คุณต้องการลบการ์ดงานนี้หรือไม่?')) {
                          handleDeleteCard(editingCard.id);
                          setIsDrawerOpen(false);
                        }
                      } else if (editingList) {
                        if (confirm('คุณต้องการลบบอร์ดโปรเจกต์นี้หรือไม่?')) {
                    ...
                    title={editingSubItem ? "ลบรายการย่อย" : editingCard ? "ลบการ์ดงาน" : "ลบบอร์ด"}
                  >
                        ...
                        {subLinkUrl && (
                          <div className="flex items-center justify-between text-indigo-700 truncate font-mono text-[11px]">
                            ...
                            <button
                              ...
                              className="text-xs font-bold underline shrink-0 ml-2 cursor-pointer"
                            >
                              เปิดลิงก์
                            </button>
                          </div>
      ...
      {showDiscardModal && (
        <div className="fixed inset-0 z-60 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden border border-slate-200 p-6 text-center space-y-4 animate-in zoom-in-95 duration-150">
            <div className="w-12 h-12 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center mx-auto">
              <AlertTriangle className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900">ทิ้งการเปลี่ยนแปลง?</h3>
              <p className="text-xs text-slate-500 mt-1">
                คุณมีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก หากคุณปิดหน้าต่างนี้ ข้อมูลที่แก้ไขจะสูญหาย
              </p>
            </div>
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 px-4 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-md transition-all active:scale-95 cursor-pointer"
              >
                ทิ้งการเปลี่ยนแปลง
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[60] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-150">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">ชื่อบอร์ดงาน (Project Name) <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={createListName}
                  onChange={(e) => setCreateListName(e.target.value)}
                  placeholder="เช่น ออกแบบหน้าเว็บ..."
                  className="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">ชื่อการ์ดงานแรก (Card Name) <span className="text-slate-400 font-normal ml-1">(เว้นว่างไว้ถ้าไม่ต้องการสร้างการ์ดตอนนี้)</span></label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">วันที่กำหนดส่ง</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-700 mb-1 block">ความสำคัญ (Priority)</label>
                  <select
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 mb-2 block">มอบหมายให้</label>
                <div className="flex flex-wrap gap-2">
                  {createListAssignees.map(uid => {
                    const u = userMap[uid];
                    if (!u) return null;
                    return (
                      <div key={u.id} className="relative group">
                        ...
                        <button
                          type="button"
                          onClick={() => setCreateListAssignees(prev => prev.filter(id => id !== u.id))}
                          className="absolute -top-1 -right-1 w-4 h-4 bg-white rounded-full flex items-center justify-center text-rose-500 hover:text-rose-700 shadow-sm border border-slate-200"
                        >
                  ...
                  <button
                    type="button"
                    onClick={() => setShowCreateInvitePopover(!showCreateInvitePopover)}
                    className="w-8 h-8 rounded-full border-2 border-dashed border-slate-300 hover:border-blue-500 hover:bg-blue-50 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-all cursor-pointer"
                ...
                {showCreateInvitePopover && (
                  <div className="mt-2 p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                      เลือกมอบหมายให้ผู้รับผิดชอบงานหลัก:
                    </span>
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        const taskAssigneeIds = task.assignee_ids && task.assignee_ids.length > 0
                        ...
                        const candidates = taskAssigneeIds
                          .map((id: string) => userMap[id])
                          .filter(Boolean)
                          .filter((u) => !createListAssignees.includes(u.id));
                        ...
                        return candidates.length > 0 ? (
                          candidates.map((u) => (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setCreateListAssignees(prev => [...prev, u.id]);
                              }}
                              className="flex items-center gap-2 px-3 py-1.5 bg-white hover:bg-blue-50 border border-slate-200 hover:border-blue-300 rounded-full text-xs font-semibold text-slate-700 hover:text-blue-700 transition-all cursor-pointer active:scale-95 shadow-2xs"
                            >
                              <img
                          ...
                          <span className="text-xs text-slate-400 italic">
                            ไม่มีรายชื่อผู้รับผิดชอบเพิ่มเติม
                          </span>
                        );
                      })()}
                    </div>
              ...
              <div>
                <label className="text-xs font-bold text-slate-700 mb-1 block">โน้ต / รายละเอียด</label>
                <textarea
                  value={createListNote}
                  onChange={(e) => setCreateListNote(e.target.value)}
                  placeholder="รายละเอียดเพิ่มเติม..."
                  className="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[80px]"
                />
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-200 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isCreatingList}
                className="inline-flex items-center gap-1.5 px-5 py-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md transition-all active:scale-95 cursor-pointer disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
                <span>{isCreatingList ? 'กำลังสร้าง...' : 'บันทึก'}</span>
              </button>
            </div>
          </div>
        </div>
