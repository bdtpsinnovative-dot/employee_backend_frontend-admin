...
export const TaskBoardView: React.FC<TaskBoardViewProps> = ({
  ...
  return (
    <div className="task-board-container p-4 md:p-6">
      {/* Mobile Column Switcher Tabs */}
      <div className="task-mobile-tabs">
        <button
          className={`task-mobile-tab-btn ${mobileActiveCol === 'all' ? 'active' : ''}`}
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <button
              key={col.id}
              className={`task-mobile-tab-btn ${mobileActiveCol === col.id ? 'active' : ''}`}
      ...
      <div className="task-board-grid grid grid-cols-1 lg:grid-cols-4 md:grid-cols-2 gap-6 items-start">
        {COLUMNS.map((col) => {
          const columnTasks = tasks.filter((t) => t.status === col.id);
          const isHovered = activeDragColumn === col.id;
          const isHiddenMobile = mobileActiveCol !== 'all' && mobileActiveCol !== col.id;
          ...
          return (
            <div
              ...
              className={`task-board-column ${
                isHiddenMobile ? 'mobile-hidden' : ''
              } flex flex-col bg-slate-50/80 rounded-2xl border transition-all p-4 min-h-[500px] shadow-2xs ${
                isHovered
